...
/**
 * 　　* @Description: 机顶盒终端状态机
 * 　　* @author ckx
 * 　　* @date 2018/9/28 15:47
 */
public class ClientStatMachine implements Serializable {

    private static Logger logger = LoggerFactory.getLogger(ClientStatMachine.class);
    /**
     * 序列化.
     */
    private static final long serialVersionUID = -874132211120578334L;
    ...
    public static int PUSH_IM_AFTER_ACTIVE_SECOND = 60;//超时时间
    ...
    static {
        String strPolicy = Constant.ACK_HEARTBEAT_POLICY;
    ...
    /**
     * * @Description: 根据机顶盒udp数据包创建机顶盒节点对象
     * * @author ckx
     * * @date 2018/12/25 15:56
     */
    public static ClientStatMachine newByClientTick(ClientMessage m) throws NullPointerException {
        // TODO: 2018/12/25 目前机顶盒只会存在心跳包
//        if (m.getCmd() != ClientStatMachine.CMD_0x00) {
//            return null;//非心跳包不产生新的状态机
//        }
        ClientStatMachine csm = new ClientStatMachine();
        //csm.lastAddr = address;不能设置该值，否则创建状态机后第一次onClientMessage不回发心跳了
        csm.lastAddr = m.getSocketAddress();
        csm.lastTick = System.currentTimeMillis();
        return csm;
    }

    public static ClientStatMachine newByClientStatMachinePojo(String lastIp, int lastPort) throws NullPointerException {

        ClientStatMachine csm = new ClientStatMachine();
        SocketAddress socketAddress = new InetSocketAddress(lastIp, lastPort);
        csm.lastAddr = socketAddress;
        csm.lastTick = System.currentTimeMillis();
        return csm;
    }
...
//		}
        ArrayList<ServerMessage> smList = new ArrayList<>();
